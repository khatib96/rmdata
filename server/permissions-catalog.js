...
/**
 * Permission Catalog v4 — Server side (MariaDB).
 * Must match src/permissions/permissionCatalogV2.ts exactly.
 * No manage, no deny, no role_permissions.
 */

const PERMISSION_CATALOG_V2 = [
  // ─── Employees ────────────────────────────────────────────
...
/**
 * Sync catalog to MariaDB. Idempotent.
 */
async function syncPermissionCatalogMySQL(query) {
  let inserted = 0;
  for (const p of PERMISSION_CATALOG_V2) {
    const res = await query(
      'INSERT IGNORE INTO permissions (module, action, labelKey) VALUES (?, ?, ?)',
      [p.module, p.action, `${p.module}.${p.action}`]
    );
    if (res && res.affectedRows) inserted++;
  }
  if (inserted) {
    console.log(`✅ Permission catalog sync (MySQL): ${inserted} new entries`);
  }
}

module.exports = {
  PERMISSION_CATALOG_V2,
  syncPermissionCatalogMySQL,
};
